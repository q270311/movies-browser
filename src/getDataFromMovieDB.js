...
export const getPersonPopular = async ({ page }) => {
    const response = await axios.get(`${baseURL}/person/popular?api_key=${apiKey}&language=en-US&page=${page}`);
    if (!response.ok) {
        new Error(response.statusText);
    }
...
export const getPersonMovieCredits = async ({ personID }) => {
    const response = await axios.get(`${baseURL}/person/${personID}/combined_credits?api_key=${apiKey}&language=en-US`);
    if (!response.ok) {
        new Error(response.statusText);
    }
...
export const searchMovie = async ({ page, query }) => {
    const response = await axios.get(`${baseURL}/search/movie?api_key=${apiKey}&language=en-US&query=${query}&page=${page}&include_adult=false`);
    if (!response.ok) {
        new Error(response.statusText);
    }
